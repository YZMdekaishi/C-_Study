using System;
using System.Collections.Generic;
using System.IO.Ports;
using System.Linq;
using System.Text;
using System.Threading;
using System.Threading.Tasks;

namespace DAL
{
    public class Modbus
    {
        //定义串口类对象
        private SerialPort MyCom;
        //定义CRC校验高低位
        private byte ucCRCHi = 0xFF;
        private byte ucCRCLo = 0xFF;
        //定义一个接收字节数组
        byte[] bData = new byte[1024];
        byte mReceiveByte;
        int mReceiveByteCount = 0;
        //定义设备地址
        int CurrentAddr;//从站地址
        int iMWordLen;//寄存区的长度
        int iMBitLen;//线圈的长度
        //定义返回报文
        string strUpData;
        public Modbus()
        {
            MyCom = new SerialPort();
        }
        #region 打开关闭串口方法
        /// <summary>这个下面这个是默认的端口属性
        /// 打开串口方法【9600 N 8 1】表示波特率9600，N表示无校验，8是数据位，1是停止位
        /// </summary>
        /// <param name="iBaudRate">波特率</param>
        /// <param name="iPortNo">端口号</param>
        /// <param name="iDataBits">数据位</param>
        /// <param name="iParity">校验位</param>
        /// <param name="iStopBits">停止位</param>
        /// <returns></returns>
        /// 
        public bool OpenMyComm(int iBaudRate,string iPortNo,int iDataBits,Parity iParity,StopBits iStopBits)
        {
            try
            {
                //关闭已打开串口
                if (MyCom.IsOpen)
                {
                    MyCom.Close();
                }
                //设置串口属性
                MyCom.BaudRate = iBaudRate;
                MyCom.PortName = iPortNo;
                MyCom.DataBits = iDataBits;
                MyCom.Parity = iParity;
                MyCom.StopBits = iStopBits;
                MyCom.ReceivedBytesThreshold = 1;//设置缓存区的字节数
                MyCom.DataReceived += new SerialDataReceivedEventHandler(MyCom_DataReceived);//表示串口接收到数据后，绑定一个事件
                
                MyCom.Open();//设置完之后打开串口 
                return true;
            }
            catch
            {
                return false;
            }

        }
         
        /// <summary>
        /// 关闭串口方法
        /// </summary>
        /// <returns></returns>
        public bool ClosePort()
        {
            if (MyCom.IsOpen)
            {
                MyCom.Close();
                return true;
            }
            else
            {
                return false;//说明关闭没关闭掉
            }
           
        }
        #endregion
        //下面这个是串口绑定的事件
        //用于获取返回的报文
        public void MyCom_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            //throw new NotImplementedException();
            while (MyCom.BytesToRead > 0)
            {
                mReceiveByte = (byte)MyCom.ReadByte();
                bData[mReceiveByteCount] = mReceiveByte;
                mReceiveByteCount += 1;
                if (mReceiveByteCount >= 1024)
                {
                    mReceiveByteCount = 0;
                    MyCom.DiscardInBuffer();//清空自来串行驱动程序中的缓存区的数据；
                    return;
                }
            }
            //读取保持型寄存器 功能码0x03
             if (bData[0] == (byte)CurrentAddr && bData[1] == 0x03 && mReceiveByteCount >= (iMWordLen * 2 + 5))
            {
                strUpData = "";
                for (int i = 0; i < iMWordLen * 2 + 5; i++)
                {
                    strUpData = strUpData + " " + bData[i].ToString("X2");//X表示转成16进制，2表示它的长度
                }
                //清除缓存区
                MyCom.DiscardInBuffer();
            }
        }

        #region 读取保持型寄存器 功能码0x03
        /// <summary>
        /// 读取保持型寄存器 功能码0x03
        /// </summary>
        /// <param name="iDevAdd">从站地址</param>
        /// <param name="iAddress">起始地址</param>
        /// <param name="iLength">长度</param>
        /// <returns></returns>

        
        public byte[] ReadKeepReg(int iDevAdd,int iAddress,int iLength)
        {
            byte[] ResByte = null;
            iMWordLen = iLength;//寄存器长度
            CurrentAddr = iDevAdd;//从站地址
            //从站地址（01）+功能码（03）+起始地址（00 00）+寄存器数（00 0A）+CRC校验位(C5 CD）
            //第一步：拼接报文
            byte[] SendCommand = new byte[8];
            SendCommand[0] = (byte)iDevAdd;//从站地址
            SendCommand[1] = 0x03;//功能码
            SendCommand[2] = (byte)((iAddress - iAddress % 256) / 256);//高位从站地址
            SendCommand[3] = (byte)(iAddress % 256);//低位从站地址
            SendCommand[4] = (byte)((iLength - iLength % 256) / 256);//高位寄存器数
            SendCommand[5] = (byte)(iLength % 256);//低位寄存器数
            Crc16(SendCommand,6);//这个地方为什么是6，有点不明白,查资料得出6表示对SendCommand的前6个字节进行CRC16校验
            SendCommand[6] = ucCRCLo;
            SendCommand[7] = ucCRCHi; 

            //第二步：发送报文
            try
            {
                MyCom.Write(SendCommand, 0, 8);
            }
            catch (Exception)
            {

                return null;
            }

            //第三步：解析报文
            mReceiveByteCount = 0;
            Thread.Sleep(100);
            ResByte = HexStringToByteArray(this.strUpData);
            return ResByte;
            

        }
        #endregion
        //
        private byte[] HexStringToByteArray(string S)
        {
            byte[] Res = null;
            if (S != null && S.Length > 5)
            {
                string[] str = S.Trim().Split(' ');
                string[] Result = new string[str.Length - 5];
                Res = new byte[str.Length - 5];
                for (int i = 0; i < str.Length-5; i++)
                {
                    Result[i] = str[i + 3];
                }
                for (int i = 0; i < Result.Length; i++)
                {
                    Res[i] = Convert.ToByte(Result[i], 16);
                }
            }
            return Res;
        }

            #region  CRC校验
        private static readonly byte[] aucCRCHi = {
             
         };
        private static readonly byte[] aucCRCLo = {
             
         };
        private void Crc16(byte[] pucFrame, int usLen)
        {
            int i = 0;
            ucCRCHi = 0xFF;
            ucCRCLo = 0xFF;
            UInt16 iIndex = 0x0000;

            while (usLen-- > 0)
            {
                iIndex = (UInt16)(ucCRCLo ^ pucFrame[i++]);
                ucCRCLo = (byte)(ucCRCHi ^ aucCRCHi[iIndex]);
                ucCRCHi = aucCRCLo[iIndex];
            }

        }

        #endregion
    }
}
